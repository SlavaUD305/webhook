    ...
    while ($row = mysqli_fetch_array($resultSelect, MYSQLI_NUM)) {
        foreach ($row as $idBitrix) {
            if (!empty($idBitrix)) {
                ...
                if (!empty($result)) {
                    $stmt = mysqli_prepare($link, "DELETE id_company_crm, id_company_bitrix FROM  companys_id WHERE id_company_crm=?, id_company_bitrix=?");
...
function delete_contact($data){
    ...
    while ($row = mysqli_fetch_array($resultSelect, MYSQLI_NUM)) {
        foreach ($row as $idBitrix) {
            ...
            if (!empty($result)) {
                $stmt = mysqli_prepare($link, "DELETE id_contact_crm, id_contact_bitrix FROM  contact_id WHERE id_contact_crm=?, id_contact_bitrix=?");
